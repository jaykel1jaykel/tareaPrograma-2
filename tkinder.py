...
def cedulaInser(frameInsertar):
    Label(frameInsertar, text = "Cedula", font=("Arial",14,"bold"),fg="#7A1027").grid(row= 0,column=0,pady=5)
    cedula = StringVar()
...
def cedulaInserAux(cedula):
    ...
    for i in numeros:
        if i.isdigit()==False:
            return(False,"Los datos ingresados in cedula nada mas deben ser numeros")
...
def nombreInser(frameInsertar):
    Label(frameInsertar, text = "Nombre completo", font=("Arial",14,"bold"),fg="#7A1027").grid(row= 1,column=0,pady=5)
    nombre = StringVar()
...
def normalizarNombre(pnombre):
    if pnombre == "":
        return (False,"Debe incuir un nombre")
...
def fechaNacimientoInser(frameInsertar):
    Label(frameInsertar, text = "Fecha de nacimiento", font=("Arial",14,"bold"),fg="#7A1027").grid(row= 2,column=0,pady=5)
...
def tipoSangreInser(frameInsertar):
    Label(frameInsertar, text = "Tipo de sangre", font=("Arial",14,"bold"),fg="#7A1027").grid(row= 3,column=0,pady=5)
...
def sexoInser(frameInsertar):
    ...
    Label(frameSexo, text = "Sexo", font=("Arial",14,"bold"),fg="#7A1027").grid(row= 0,column=0,pady=5)
    sexo = BooleanVar()
...
def pesoInser(frameInsertar):
    Label(frameInsertar, text = "Peso(kg)", font=("Arial",14,"bold"),fg="#7A1027").grid(row= 5,column=0,pady=5)
    peso = StringVar()
...
def validarPesoAux(peso):
    ...
    peso=int(peso)
    if peso<50:
        return(False,"El peso minimo permitido es 50 kg")
    if peso>120:
        return(False,"El peso maximo permitido es 120 kg")
    return(True,peso)

def telefonoInser(frameInsertar):
    Label(frameInsertar, text = "Telefono", font=("Arial",14,"bold"),fg="#7A1027").grid(row= 6,column=0,pady=5)
    telefono = StringVar()
    Entry(frameInsertar,textvariable= telefono).grid(row=6,column=1)
    return telefono

def validarTelefonoAux(telefono):
    formato=r"[246789]{1}-?[0-9]{4}-?[0-9]{4}"
    if re.fullmatch(formato,telefono)==False:
...
def correoInser(frameInsertar):
    Label(frameInsertar, text = "Correo", font=("Arial",14,"bold"),fg="#7A1027").grid(row= 7,column=0,pady=5)
    correo = StringVar()
    Entry(frameInsertar,textvariable=correo).grid(row=7,column=1)
    return correo

def validarCorreoAux(correo):
    formato=r"[a-zA-Z0-9._%+-]+@(gmail\.com|hotmail\.com|yahoo\.com|outlook\.com|icloud\.com)$"
    if re.fullmatch(formato,correo)==False:
...
def mayorEdad(fechaNacimiento):
    ...
    return"Dado su fecha de nacimiento usted aun no piede ser donador"
...
def validarLugarNacimiento(cedula):
    ...
    return "Dado que usted nació en la provincia de:"+provincia+"usted podria donar en:"+ubicacion
...
def registrar(ventanaInsertar,donantes,cedula,nombre,fechaNacimiento,tipoSangre,sexo,peso,telefono,correo,mensaje):
...
def crearBotones(frameBotones,ventanaInsertar,mensaje,donantes,cedula,nombre,fechaNacimiento,tipoSangre,sexo,peso,telefono,correo):
    Button(frameBotones,text="Registrar",font=("Arial",12,"bold"),
        bg="#8B1E3F",fg="white",
        command=lambda: registrar(ventanaInsertar,donantes,cedula,nombre,fechaNacimiento,
            tipoSangre,sexo,peso,telefono,correo,mensaje)
    ).grid(row=0,column=0,padx=10)
    Button(frameBotones,text="Limpiar",
        font=("Arial",12,"bold"),bg="#8B1E3F",fg="white",
...
def insertarDonador(donantes):
    ...
    ventanaInsertar.geometry("700x900")
    ventanaInsertar.config(bg="#F5F5F5")
    frameInsertar=Frame(ventanaInsertar,bg="#FFFFFF",bd=5)
    ...
    crearBotones(frameBotones,ventanaInsertar,mensaje,donantes,cedula,nombre,fechaNacimiento,tipoSangre,sexo,peso,telefono,correo)
...
Button(frameMenu,text="3. Actualizar datos",font=("Arial",14),
    width=30,height=2,bg="#1565c0",fg="white",command=actualizar).grid(row=2,column=0)
